    ...
    <el-table v-loading="loading" :data="jobList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="任务组名" align="center" prop="jobGroup">
        <template #default="scope">
          <dict-tag :options="sysJobGroup" :value="scope.row.jobGroup" />
        </template>
      </el-table-column>
      <el-table-column
        label="调用目标字符串"
          ...
          <el-col :span="12">
            <el-form-item label="状态">
              <el-radio-group v-model="form.status">
                <el-radio v-for="dict in sysJobStatus" :key="dict.value" :label="dict.value">{{
                  dict.label
                }}</el-radio>
              </el-radio-group>
            </el-form-item>
...
<script setup name="Job">
  ...
  const { sys_job_group: sysJobGroup, sys_job_status: sysJobStatus } = proxy.useDict(
    'sys_job_group',
    'sys_job_status'
  )
  ...
  function jobGroupFormat(row) {
    return proxy.selectDictLabel(sysJobGroup.value, row.jobGroup)
  }
  /** 取消按钮 */
  function cancel() {
    open.value = false
